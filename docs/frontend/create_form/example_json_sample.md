...
- _important - служебное поле наличие которого имеет значение при формировании json ответа
...
- _obj - указывает на первый объект в шаблоне, и на контейнер объектов в форме (в примере "scene").
...
#### В ответе (json)
В выходном JSON массив представлен не как массив, а как словарь ключами которого являются окончания пути к объектам. Пример "scenes.scene_1" -> "scene_1".
```json
  name: {"scene_1":..., "scene_2":...}
```
