...
Для свойств: for, data-path, id, data-target, data-parent элементов могут содержать пути в которых шаблон нужно поменять на другой, в глубину по всей иерархии, как в самом активном элементе, так и во вложенных. Например в пути: scenes.scene_1.title меняем scene_1 на scene_2 и получаем scenes.scene_2.title.
Свойство data-id только активного элементов содержит индекс, для scene_1 там будет 1, для scene_2 меняем на 2.
В первом элементе вниз по иерархии от активного элемента со свойством data-role="title" значение состоит из 2 частей, названия и индекса (через пробел), например Сцена 1. Находим число внутри значения и меняем на новое значение data-id.
...
- Если удаляемый объект один в контейнере то вместо удаления он очищается функцией Очистить.
- Сдвигает объекты функцией "shiftDown"
